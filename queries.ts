...
const createUser = (req, res) => {
  ...
  pool.query(
    "INSERT INTO users (handle, displayname) VALUES ($1, $2) RETURNING *",
    [handle, displayName],
    (error, results) => {
      if (error) {
        throw error;
...
const updateUser = (req, res) => {
  ...
  pool.query(
    "UPDATE users SET handle = $1, displayname = $2 WHERE id = $3",
...
const getPostsByUser = (req, res) => {
  ...
  pool.query(
    "SELECT posts.id AS postID, posts.content, posts.time, users.displayname, users.handle FROM posts JOIN users ON userid = users.id WHERE userid = $1 ORDER BY time DESC",
    [userID],
    (error, results) => {
      if (error) {
        throw error;
...
const createPost = (req, res) => {
  ...
  pool.query(
    "INSERT INTO posts (userid, time, content) VALUES ($1, now(), $2) RETURNING *",
    [userID, content],
    (error, results) => {
      if (error) {
        throw error;
